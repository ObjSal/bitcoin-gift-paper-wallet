...
Both servers expose 9 tools: `generate_segwit_wallet`, `generate_taproot_wallet`, `check_balance`, `check_all_balances`, `sweep_wallet`, `recover_wallet`, `open_wallet_app`, `list_generated_wallets`, `open_wallet_bill`. Generated bills are saved to `generated-bills/` in the project root. Metadata JSON files are always saved alongside bill PNGs (enabling `check_all_balances` and rich `list_generated_wallets` output). Sweep and recover tools support optional tips (percentage or fixed sats) to a project donation address. Both servers support regtest via `REGTEST_SERVER_URL` environment variable. The `.mcpb` bundle ships with `REGTEST_SERVER_URL=http://127.0.0.1:8080` pre-configured; users can override it via Claude Desktop config env.

**MCP Apps (inline bill preview):** The Node.js MCP server implements the MCP Apps protocol to render bill images inline in Claude Desktop conversations. It registers a `ui://bitcoin-gift-wallet/bill-viewer.html` resource with MIME type `text/html;profile=mcp-app`. Tool results include `structuredContent` with a shrunk preview image (600px wide, ~150KB base64) and metadata. The MCP App HTML performs the JSON-RPC handshake (`ui/initialize` → `ui/notifications/initialized` → `ui/notifications/size-changed`) and renders the bill with address, type badge, and click-to-open-full-size functionality. The full-size bill is not included in tool results (only structuredContent) to stay under the 1MB tool result size limit. The `_meta.ui.resourceUri` field in tool results links them to the MCP App resource.
